...
from crawler_sys.utils import connect_with_redis
...
class Crawler_v_qq():
    ...
    def lst_page_task(self, lst_page_url,
                      page_num_max=34):
        lst_page_task_lst = []
        videos_in_one_page = 30
        num_lst = []
        for i in range(0, page_num_max):
            ...
            num_lst.append(num)
        task_url_lst_new = [lst_page_url + '/?sort=5&offset=' + str(num) for num in num_lst]
        lst_page_task_lst.extend(task_url_lst_new)
        task_url_lst_hot = [lst_page_url + '/?sort=40&offset=' + str(num) for num in num_lst]
        lst_page_task_lst.extend(task_url_lst_hot)
        task_url_lst_praise = [lst_page_url + '/?sort=48&offset='+ str(num) for num in num_lst]
        lst_page_task_lst.extend(task_url_lst_praise)
        return lst_page_task_lst


    async def lst_page(self, loop, task_lst, channel):
        url_lst = []
        count = 0
        async with aiohttp.ClientSession() as sess_lst_page:
            task_video_page = [loop.create_task(self.asynchronous_get_lst_page(sess_lst_page, lst_url)) for lst_url in task_lst]
            ...
            for lst_html in lst_page_download_result_lst:
                url_lst_partial = self.process_lst_page(resp=lst_html, channel=channel)
    ...
    def process_lst_page(self, resp, channel):
        video_lst = []
    ...
    async def asynchronous_get_lst_page(self, session, url):
    ...
    def run_lst_page_asyncio(self, lst_page_url,
                             page_num_max=34):
        """
        get list page asynchronously
        """
        channel = lst_page_url.split("/")[-1]
        task_lst = self.lst_page_task(lst_page_url=lst_page_url, 
                                      page_num_max=page_num_max, channel=channel)
        print('the length of task list is %s' % len(task_lst))
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self.lst_page(loop, task_lst=task_lst))


    def run_video_page_asyncio(self):
        """
        get video page asynchronously
        """
    ...
    def parse_video_page_multiprocess(self, para_dic):
        pool = Pool(processes=8)
        for line in range(8):
            pool.apply_async(self.parse_video_page_single_process, **para_dic)
        pool.close()
        pool.join()
